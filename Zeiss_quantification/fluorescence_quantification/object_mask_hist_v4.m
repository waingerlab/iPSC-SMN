...
%in version 4, I changed the relative size measurement cutoff to a hard
%cutoff that I wrote based on the nuclear size distribution
function [objectarray, objectmask, objectnumber, binarymask, estimatedmissedcells, meansize, multiobjectcenters] = object_mask_hist_v4(channel_matrix,minimumradius,stdevs)

dimensions = size(channel_matrix);
binarymask = histmask_v3(channel_matrix,stdevs);% calculates mask using histmask method
...
multinuclear1 = cellsize<900;
multinuclear2 = cellsize>2500;
multinuclear = logical(multinuclear1+multinuclear2);
%multinuclear = find(cellsize>(2.5*mediansize));%no objects larger than 80,000 pixels included- no accurate estimate from these
